...
TEST_CASE(quat_to_mat4_test, "[quaternion]") {
    const quaternion q = quat_from_euler((vector3){.x = 0.0f, .y = 0.0f, .z = 90.0f});
...
TEST_CASE(quat_conjugate_inverse, "[quaternion]") {
    const quaternion q = quat_from_axis_angle((vector3){.x = 0.0f, .y = 0.0f, .z = 1.0f}, 90.0f);
...
TEST_CASE(quat_axis_angle_roundtrip, "[quaternion]") {
    ...
    const vm_float_t deg_in = 90.0f;

    const quaternion q = quat_from_axis_angle(axis_in, deg_in);

    vm_float_t degrees = 0.0f;
    const vector3 axis = quat_to_axis_angle(q, &degrees);
    const quaternion q2 = quat_from_axis_angle(axis, degrees);
...
TEST_CASE(quat_euler_roundtrip, "[quaternion]") {
    const vector3 e = {.x = (vm_float_t)0.0, .y = (vm_float_t)0.0, .z = (vm_float_t)90.0};
...
TEST_CASE(quat_mat_roundtrip, "[quaternion]") {
    const quaternion q = quat_from_axis_angle((vector3){.x = 0.0f, .y = 1.0f, .z = 0.0f}, 45.0f);
...
TEST_CASE(quat_slerp_nlerp, "[quaternion]") {
    ...
    const quaternion b = quat_from_axis_angle((vector3){.x = 0.0f, .y = 0.0f, .z = 1.0f}, 90.0f);
