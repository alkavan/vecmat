...
void mat4_mul_vec4_ptr_avx(vector4 *res, const matrix4 *m, const vector4 *v)
{
    ...
    r = _mm256_add_pd(r, _mm256_mul_pd(_mm256_loadu_pd(&m->v[4]), _mm256_set1_pd(v->y)));
    r = _mm256_add_pd(r, _mm256_mul_pd(_mm256_loadu_pd(&m->v[8]), _mm256_set1_pd(v->z)));
    r = _mm256_add_pd(r, _mm256_mul_pd(_mm256_loadu_pd(&m->v[12]), _mm256_set1_pd(v->w)));
...
void mat4_mul_vec3_ptr_avx(vector3 *res, const matrix4 *m, const vector3 *v, const vm_float_t w)
{
    ...
    r = _mm256_add_pd(r, _mm256_mul_pd(_mm256_loadu_pd(&m->v[4]), _mm256_set1_pd(v->y)));
    r = _mm256_add_pd(r, _mm256_mul_pd(_mm256_loadu_pd(&m->v[8]), _mm256_set1_pd(v->z)));
    r = _mm256_add_pd(r, _mm256_mul_pd(_mm256_loadu_pd(&m->v[12]), _mm256_set1_pd(w)));
    double t[4];
...
void mat4_mul_vec4_ptr_avx(vector4 *res, const matrix4 *m, const vector4 *v)
{
    ...
    r = _mm_add_ps(r, _mm_mul_ps(_mm_loadu_ps(&m->v[4]), _mm_set1_ps(v->y)));
    r = _mm_add_ps(r, _mm_mul_ps(_mm_loadu_ps(&m->v[8]), _mm_set1_ps(v->z)));
    r = _mm_add_ps(r, _mm_mul_ps(_mm_loadu_ps(&m->v[12]), _mm_set1_ps(v->w)));
...
void mat4_mul_vec3_ptr_avx(vector3 *res, const matrix4 *m, const vector3 *v, const vm_float_t w)
{
    ...
    r = _mm_add_ps(r, _mm_mul_ps(_mm_loadu_ps(&m->v[4]), _mm_set1_ps(v->y)));
    r = _mm_add_ps(r, _mm_mul_ps(_mm_loadu_ps(&m->v[8]), _mm_set1_ps(v->z)));
    r = _mm_add_ps(r, _mm_mul_ps(_mm_loadu_ps(&m->v[12]), _mm_set1_ps(w)));
    float t[4];
    _mm_storeu_ps(t, r);
    res->x = t[0];
    res->y = t[1];
